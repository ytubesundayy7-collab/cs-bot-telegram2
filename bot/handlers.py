...
async def handle_source_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if str(chat.id).startswith("-100"):
        # Supergroup link format
        group_id_part = str(chat.id)[4:]  # Remove -100
    ...
    try:
        ...
        # We store the action message ID as the main operator_message_id for reply tracking
        async with AsyncSessionLocal() as session:
            service = TicketService(session)
...
async def handle_operator_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Find the ticket - check if reply is to our action message or forwarded message
    async with AsyncSessionLocal() as session:
        service = TicketService(session)
        ...
        # Forwarded messages have forward_from_chat and forward_from_message_id
        if not ticket and reply_to.forward_from_chat:
            # Find by source chat and message ID
